...
<script> setActive("fishgrid"); </script>
	...
	<div class="container">
		<div class="row">
			<div class="col justify-content-between align-items-center text-center">
				...
				<button class="bmargin btn btn-sm btn-outline-dark" type="button" onclick="jump('reg_fishing_list.php')"><i class="fa fa-list tpad"></i> </i> <x class="thid">List View</x></button>
				<select class="swid bmargin btn btn-sm btn-outline-dark" onchange="if(this.value=='All barangays')jump('reg_fishing_grid.php'); else jump('reg_fishing_grid.php?barangays='+this.value+'')">
				...
				<button class="bmargin btn btn-sm btn-outline-info" type="button" onclick="getID('t_search').value='';jump('reg_fishing_grid.php')"><i class="fa fa-sync tpad"></i> <x class="thid">Refresh</x></button>
				<?php
					if(!isset($_SESSION['user'])){
						echo"";
					}else
						echo"
					<a rel='facebox' href='reg_fishing_add.php'><button class='bmargin btn btn-sm btn-outline-success' type='button'><i class='fa fa-plus tpad'></i> <x class='thid'>Add MFVR</x></button></a>";
				?>			
			</div>				
		</div>
	</div>
		...
		<?php
			...
			while($rs=mysqli_fetch_array($ex)){
				...
				if(isset($_POST["b_remove_$rs[0]"])){
					$link->query("delete from reg_fishing where idn='$rs[0]'");
					jump("");
				}
				
				if(isset($_POST["b_upImg_$rs[0]"])){
					move_uploaded_file($_FILES["b_file_$rs[0]"]["tmp_name"], "images/reg_fishing/$rs[0].jpg");
					$link->query("update reg_fishing set ispicset=1 where idn='$rs[0]'");
					jump("");
				}

				$photo_path = 'images/photogallery.png';
				$img_style = "opacity: 0.3;";
				if(file_exists("images/reg_fishing/$rs[0].jpg")){
					$photo_path = "images/reg_fishing/$rs[0].jpg?" . date("h:i:s");
					$img_style = "";
				}
				...
				<div class="col-xl-3 col-lg-4 col-md-6 col-sm-12 mb-4" id="div_<?php echo $rs[0]; ?>">
					<div class="card fishing-card h-100 shadow-sm border-2">
						<div class="fishing-card-img-container">
							<!-- Index Badge -->
							...
							<!-- Image -->
							<img src="<?php echo $photo_path; ?>" alt="Fishing Vessel Photo" onclick="jump('reg_fishing_pds.php?reg_fishing=<?php echo $rs[0]; ?>')" style="cursor:pointer; <?php echo $img_style; ?>" />
							
							<!-- Hover actions overlay -->
							<div class="fishing-hover-actions">
								<button type="button" class="btn btn-sm btn-primary btn-block my-1" onclick="jump('reg_fishing_pds.php?reg_fishing=<?php echo $rs[0]; ?>')">
									<i class="fas fa-print mr-1"></i> Print MFVR
								</button>
								<button type="button" class="btn btn-sm btn-info btn-block my-1" onclick="jump('permit_fishing_pds.php?reg_fishing=<?php echo $rs[0]; ?>')">
									<i class="fas fa-print mr-1"></i> Print Permit
								</button>
								<?php if ($is_admin): ?>
									<button type="button" class="btn btn-sm btn-light btn-block my-1" onclick="$('#b_file_<?php echo $rs[0]; ?>').click();">
										<i class="fas fa-camera mr-1"></i> Change Photo
									</button>
									<a rel="facebox" href="reg_fishing_edit_form.php?reg_fishing=<?php echo $rs[0]; ?>" class="btn btn-sm btn-warning btn-block my-1">
										<i class="fas fa-edit mr-1"></i> Edit Details
									</a>
									<button type="button" class="btn btn-sm btn-danger btn-block my-1" onclick="deleteRecord('reg_fishing', <?php echo $rs[0]; ?>, 'div_<?php echo $rs[0]; ?>')">
						...
						<div class="card-body p-3 d-flex flex-column justify-content-between">
							<div>
								...
								<h6 class="font-weight-bold mb-1 text-uppercase text-dark text-truncate">
									F/B <?php echo str_replace($search_val, $rep, $rs["tradename"]); ?>
								</h6>
								
								<!-- Case / Permit No -->
								<div class="small mb-1 text-truncate">
									<span class="text-muted">FB No:</span>
								...
								<div class="small text-muted mb-1 text-truncate">
									Color: <strong class="text-dark"><?php echo str_replace($search_val, $rep, $rs["fvcolor"]); ?></strong>
								</div>
								
								<div class="small text-muted mb-1 text-truncate">
									Engine Make: <strong class="text-dark"><?php echo str_replace($search_val, $rep, $rs["enginemake"]); ?></strong>
								</div>
								...
								<div class="mt-2 pt-2 border-top">
									<div class="d-flex align-items-center justify-content-between">
										...
										<select class="form-control form-control-sm border-0 font-weight-bold status-select" data-table="reg_fishing" data-id="<?php echo $rs['idn']; ?>" style="width: 110px; border-radius: 20px; font-size: 11px; position: relative; z-index: 10 !important;">
