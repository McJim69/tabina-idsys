<?php
	...
	$ex=$link->query("select * from senior where (
	    idn like'%".$value."%' or
	...
	$ex1=$link->query("select * from senior where (idn like'%".$value."%' or barangay like'%".$value."%') $bar $age $pen order by name_fam");				
?>
	...
	<div class="container">
		<div class="row">
			<div class="col justify-content-between align-items-center text-center">
				...
				<button class="bmargin btn btn-sm btn-outline-success" type="button" onclick="getID('t_search').value='';jump('senior_list.php')"><i class="fa fa-sync tpad"></i> <x class="thid">Refresh</x></button>
				<button class="bmargin btn btn-sm btn-outline-dark" type="button" onclick="jump('senior_grid.php')"><i class="fa fa-th tpad"></i> <x class="thid">Card View</x></button>
				<select class="swid spad bmargin btn btn-sm btn-outline-dark" onchange="if(this.value=='All barangays')jump('senior_list.php'); else jump('?barangay='+this.value)" >
				...
				<button class="bmargin btn btn-sm btn-outline-danger" type="button" onclick="jump('senior_list_80up.php')"><i class="fa fa-wheelchair tpad"></i> <x class='thid'>SC 80 UP</x></button>
				<?php
				...
				?>			
				<select class="thid spad bmargin btn btn-sm btn-outline-primary" onchange="if(this.value=='All ages')jump('senior_list.php'); else jump('?age='+this.value+'&barangay=<?php echo $_GET["barangay"];?>&pensioner=<?php echo $_GET["pensioner"];?>')" >
					<option>Ages</option>
					<?php
    ...
    <div class="card border-0 shadow-sm rounded-lg overflow-hidden">
        <div class="card-header bg-primary text-white py-3 d-flex align-items-center justify-content-between">
            <h6 class="mb-0 font-weight-bold">
                <i class="fas fa-users mr-2"></i>Senior All
            </h6>
                ...
                <?php
                    ...
                    while($rs=mysqli_fetch_array($ex)){
                        if($ctr<=$rec){
                            ...
                            echo "<td class='text-center'><span class='badge badge-primary px-2 py-1'>".sprintf("%04d", $rs["assoc_id_no"])."</span></td>";
