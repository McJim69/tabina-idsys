...
<script>setActive("sentats");</script>
...
<div class="container py-4" style="margin-top:50px;">
	...
	<div class="d-flex align-items-center justify-content-between mb-4 flex-wrap">
		<div>
			<h2 class="font-weight-bold text-dark mb-1">
				<i class="fas fa-chart-pie mr-2 text-primary"></i>Senior <x class="thid">Citizen</x> Statistics
			</h2>
	...
	<div class="row">
		...
		<div class="col-xl-3 col-md-6 col-sm-12">
			<div class="stats-card card-purple">
				...
				<div class="d-flex align-items-baseline mt-1">
					<h2 class="mb-0 font-weight-bold" style="font-size: 1.7rem;"><?php echo number_format($males_count); ?>M <?php echo number_format($females_count); ?>F</h2>
				</div>
			</div>
		</div>
	</div>
	...
	<div class="row">
		...
		<div class="col-md-4 col-sm-12">
			<div class="chart-container">
				<h4 class="chart-title">Top Barangays (Seniors)</h4>
...
<script>
	...
	var ageLineChartInstance, bracketsBarChartInstance, genderDoughnutChartInstance, pensionerPieChartInstance;
	...
	ageLineChartInstance = new Chart(ctxLine, {
		type: 'line',
		data: {
			labels: ageLabels,
			datasets: [{
		...
		options: {
			...
			scales: {
				x: {
					title: { display: true, text: 'Age (Years Old)', color: config.textColor, font: { weight: 'bold' } },
					grid: { color: config.gridColor },
					ticks: { color: config.textColor }
				},
				y: {
					title: { display: true, text: 'Number of Citizens', color: config.textColor, font: { weight: 'bold' } },
					beginAtZero: true,
					grid: { color: config.gridColor },
					ticks: { color: config.textColor, stepSize: 5 }
				}
			}
		}
	});
	...
	bracketsBarChartInstance = new Chart(ctxBar, {
		type: 'bar',
		data: {
		...
		options: {
			...
			scales: {
				...
				y: {
					beginAtZero: true,
					title: { display: true, text: 'Seniors Count', color: config.textColor, font: { weight: 'bold' } },
					grid: { color: config.gridColor },
					ticks: { color: config.textColor }
				}
			}
	...
	genderDoughnutChartInstance = new Chart(ctxDoughnut, {
		type: 'doughnut',
		data: {
			labels: ['Male', 'Female'],
			datasets: [{
	...
	pensionerPieChartInstance = new Chart(ctxPie, {
		type: 'pie',
		data: {
	...
	function updateChartThemes() {
		...
		var charts = [ageLineChartInstance, bracketsBarChartInstance, genderDoughnutChartInstance, pensionerPieChartInstance];
