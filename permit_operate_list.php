...
<?php		
	...
	$ex=$link->query("select * from permit_operate l where (
		l.idn like'%".$_POST["b_search"]."%' or
		l.name_fam like'%".$_POST["b_search"]."%' or
		l.name_1st like'%".$_POST["b_search"]."%' or
		l.name_mid like'%".$_POST["b_search"]."%' or
		l.tradename like'%".$_POST["b_search"]."%' or
		l.purok like'%".$_POST["b_search"]."%' or
		l.barangay like'%".$_POST["b_search"]."%' or
		l.city_mun like'%".$_POST["b_search"]."%' or
		l.province like'%".$_POST["b_search"]."%') order by idn DESC");
	}

	else

	$ex=$link->query("select * from permit_operate l order by idn DESC");
...
<script> setActive("operatelist"); </script>
	...
	<div class="container">
		<div class="row">
			<div class="col justify-content-between align-items-center text-center">				
				...
				<button class="bmargin btn btn-sm btn-secondary" type="button" onclick="jump('permit_operate_grid.php')"><i class="fa fa-th tpad"></i> <x class="thid"> Card View</x></button>		
				<select class="swid spad bmargin btn btn-sm btn-outline-primary" onchange="if(this.value=='All barangays')jump('permit_operate_list.php'); else jump('permit_operate_list.php?barangays='+this.value+'')">
				...
				<button class="bmargin btn btn-sm btn-info" type="button" onclick="getID('t_search').value='';jump('permit_operate_list.php')"><i class="fa fa-sync tpad"></i> <x class="thid"> Refresh</x></button>
				<a rel="facebox" href='permit_operate_add.php'><button class="bmargin btn btn-sm btn-success" type='button'><i class="fa fa-plus tpad"></i> <x class="thid">  Add Permit</x></button></a>
        ...
        <table align="center" style="margin:0 auto;">
            <tr>
                ...
                <td class="px-3 text-center">
                    <b style="font-size:16px;text-transform:uppercase">Permit to Operate Registry</b>
    ...
    <div class="card border-0 shadow-sm rounded-lg overflow-hidden">
        <div class="card-header bg-warning text-dark py-3 d-flex align-items-center justify-content-between">
            <h6 class="mb-0 font-weight-bold">
                <i class="fas fa-cogs mr-2"></i>Permit <x class="thid">to Operate (PTO) Registry</x>
            </h6>
            ...
            <table class="table table-hover table-striped align-middle mb-0" style="font-size: 13px;">
                <thead class="thead-dark text-uppercase font-weight-bold">
                    <tr>
                        ...
                        <th width="22%">Trade <x class="thid">Name</x></th>
                        <th width="8%" class="text-center">PTO <x class="thid">ID</x></th>
                        <th width="18%">Nature <x class="thid">of Activity</x></th>
                        <th width="18%">Owner <x class="thid">Name</x></th>
                        <th width="10%" class="text-center">O.R. <x class="thid">No</x></th>
                ...
                <?php
                    ...
                    while($rs=mysqli_fetch_array($ex)){
                        if($ctr<=$rec){
                            ...
                            echo "<tr class='tr-hover' style='cursor:pointer;' onclick=\"jump('permit_operate_pds.php?permit_operate=".$rs[0]."')\">";
                            echo "<td class='text-center font-weight-bold text-muted'>$i.</td>";
                            echo "<td class='font-weight-bold text-dark text-uppercase'>$trade_disp</td>";
                            echo "<td class='text-center'><span class='badge badge-warning px-2 py-1'>".sprintf("%04d", $rs[0])."</span></td>";
                            echo "<td class='font-weight-bold text-primary'>".($rs["activity"] ? htmlspecialchars($rs["activity"]) : "N/A")."</td>";
