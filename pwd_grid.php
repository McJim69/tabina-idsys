...
<script>setActive("pwdgrid");</script>
...
<div class="t_controls" style="background:url('images/bg.jpg');border:0px;padding-top:15px;padding-bottom:15px">
	<div class="container justify-content-between align-items-center text-center">
		<div class="row">
			<div class="col">
				<input class="swid bmargin btn btn-sm btn-outline-dark" placeholder="Type a keyword" type="text" name="t_search" id="t_search" value="<?php echo htmlspecialchars(($_POST["t_search"] !== "") ? $_POST["t_search"] : (isset($_GET["value"]) ? $_GET["value"] : ''), ENT_QUOTES); ?>" />
				...
				<button class="bmargin btn btn-sm btn-outline-info" type="button" onclick="jump('pwd_list.php')"><i class="fa fa-list tpad"></i> <x class="thid">List View</x></button>
				<select class="swid spad bmargin btn btn-sm btn-outline-primary" onchange="if(this.value=='All barangays')jump('pwd_grid.php'); else jump('pwd_grid.php?barangays='+this.value+'')">
				...
				<button class="bmargin btn btn-sm btn-outline-info" type="button" onclick="getID('t_search').value='';jump('pwd_grid.php')"><i class="fa fa-sync tpad"></i> <x class="thid">Refresh</x></button>
				<?php
...
<div class="container py-4 grid">
	<div class="row"> 
		<?php
			$search_val = strtoupper(($_POST["t_search"] !== "") ? $_POST["t_search"] : (isset($_GET["value"]) ? $_GET["value"] : ''));
			...
			while($rs=mysqli_fetch_array($ex)){
				...
				if(isset($_POST["b_remove_$rs[0]"])){
					$link->query("delete from pwd where idn='$rs[0]'");
					jump("");
				}
				
				if(isset($_POST["b_upImg_$rs[0]"])){
					move_uploaded_file($_FILES["b_file_$rs[0]"]["tmp_name"], "images/pwd/$rs[0].jpg");
					$link->query("update pwd set ispicset=1 where idn='$rs[0]'");
					jump("");
				}
				...
				<div class="col-xl-3 col-lg-4 col-md-6 col-sm-12 mb-4" id="div_<?php echo $rs[0]; ?>">
					<div class="card pwd-card h-100 shadow-sm border-2">
						<div class="pwd-card-img-container">
							<!-- Index Badge -->
							...
							<!-- Image -->
							<img src="<?php echo $photo_path; ?>" alt="PWD Photo" onclick="jump('pwd_pds.php?pwd=<?php echo $rs[0]; ?>')" style="cursor:pointer;" />
							
							<!-- Hover actions overlay -->
							<div class="pwd-hover-actions">
								<button type="button" class="btn btn-sm btn-primary btn-block my-1" onclick="jump('pwd_pds.php?pwd=<?php echo $rs[0]; ?>')">
									<i class="fas fa-eye mr-1"></i> View Profile
								</button>
								<?php if ($is_admin): ?>
									<button type="button" class="btn btn-sm btn-info btn-block my-1" onclick="jump('pwd_idcard1.php?pwd=<?php echo $rs[0]; ?>')">
									...
									<a rel="facebox" href="pwd_edit_form.php?pwd=<?php echo $rs[0]; ?>" class="btn btn-sm btn-warning btn-block my-1">
										<i class="fas fa-edit mr-1"></i> Edit Profile
									</a>
									<button type="button" class="btn btn-sm btn-danger btn-block my-1" onclick="deleteRecord('pwd', <?php echo $rs[0]; ?>, 'div_<?php echo $rs[0]; ?>')">
										<i class="fas fa-trash-alt mr-1"></i> Remove PWD
									</button>
								<?php endif; ?>
							</div>
						</div>
						...
						<div class="card-body p-3 d-flex flex-column justify-content-between">
							<div>
								...
								<h6 class="font-weight-bold mb-1 text-uppercase text-dark text-truncate">
									<?php echo str_replace($search_val, $rep, $rs["name_1st"] . " " . $rs["name_mid"] . " " . $rs["name_fam"]); ?>
								</h6>
								
								<!-- Position -->
								<div class="small font-weight-bold text-primary mb-2">
									<i class="fas fa-user-tag mr-1"></i><?php echo str_replace($search_val, $rep, ($rs["position"] ? $rs["position"] : "Member")); ?>
								</div>
								...
								<div class="small mb-2 text-truncate">
									...
									<strong class="text-info"><?php echo str_replace($search_val, $rep, $rs["disability"]); ?></strong>
								</div>
							...
							<div class="small text-muted border-top pt-2">
								...
								<strong><?php echo str_replace($search_val, $rep, $rs["mobileno"]); ?></strong>
							</div>
							...
							<div class="mt-2 pt-2 border-top">
								<div class="d-flex align-items-center justify-content-between">
									...
									<select class="form-control form-control-sm border-0 font-weight-bold status-select" data-table="pwd" data-id="<?php echo $rs['idn']; ?>" style="width: 110px; border-radius: 20px; font-size: 11px; position: relative; z-index: 10 !important;">
