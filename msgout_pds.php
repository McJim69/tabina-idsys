	...
	<div class="container">
		<div class="row">
			<div class="col justify-content-between align-items-center text-center">
				<a href='msgout_grid.php'><img src='images/back.png' height='30' title='Back'></a>
					&nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp;
			...
			<?php		
				$rec=1;
					$p=$_GET['page'];
						...
						$vis="";
						if($_GET["msgout"]!="")
							$vis=" and idn='".$_GET["msgout"]."' ";
																			
						$ex=$link->query("select * from msgout where idn=idn $vis order by idn limit $from,$to ");
						
						while($rs=mysqli_fetch_array($ex)){
																				
						$ex=$link->query("select * from msgout where msgout.idn='$rs[0]' and msgout.idn=msgout.idn ");
										
						while(mysqli_fetch_array($ex)){
				
						$eee=$link->query("select * from msgout l where idn='".$rs["idn"]."'");
						...
						while($rsm=mysqli_fetch_array($ex)){
							...
							echo"
								<div style='position:relative;cursor:pointer;padding-bottom:40px' onclick='printF()'>
									<img src='images/msgout/$rs[0].jpg' style='width:100%;border:2px solid #bbb;border-radius:20px'/>							
									<div>
										<div class='thid' style='position:absolute;top:20px;right:-50px'>
											<img src='images/msgout/qrcodes/$rs[0].png' width='50%'/>
										</div>
									</div>
								</div>";
							} else {
