...
if (isset($_GET['ajax']) && $_GET['ajax'] === '1') {
	require("connect.php");
	$room_id = isset($_GET['chat_room']) ? intval($_GET['chat_room']) : 0;
	...
	if(!isset($_SESSION['user'])){
		...
		$grow = ($gex instanceof mysqli_result) ? $gex->fetch_array() : null;
		$gid = isset($grow["gid"]) ? $grow["gid"] : '';
		$currentUser = "Guest-$gid";
	} else {
		$currentUser = $_SESSION['user'];
	}

	$query = $link->query("SELECT * FROM chat_messages WHERE room_id = $room_id ORDER BY id ASC");
	while ($msgRow = mysqli_fetch_array($query)) {
		...
		if ($isSelf) {
			...
			echo "<div class='chat-msg-row d-flex justify-content-end mb-3'>";
			...
			echo "<div class='chat-msg-row d-flex justify-content-start mb-3'{$recentCallAttr}>";
...
if (isset($_GET['ajax_users']) && $_GET['ajax_users'] === '1') {
	...
	if (!$extg) {
		error_log("Database error in room.php line 164: " . $link->error);
	}
	...
	if (!$extu) {
		error_log("Database error in room.php line 167: " . $link->error);
	}
	...
	echo '<div class="d-flex justify-content-between align-items-center mb-3">
		<span class="font-weight-bold text-success">Total Active:</span>
		<span class="badge badge-success px-3 py-1 font-weight-bold h6 mb-0">' . number_format($totals) . '</span>
	</div>
...
$chat = "";
if(isset($_GET["chat_room"]) && $_GET["chat_room"] != "") {
	$chat = " AND id='" . mysqli_real_escape_string($link, $_GET["chat_room"]) . "' ";
}

$sess = "";
if(isset($_GET["session"]) && $_GET["session"] != "") {
	$sess = " AND sess='" . mysqli_real_escape_string($link, $_GET["sess"]) . "' ";
}
	
$ex = $link->query("SELECT * FROM `chat_room` u WHERE id=id $chat ORDER BY id LIMIT $from,$to ");
$rs = $ex ? mysqli_fetch_array($ex) : null;
$room_id = isset($rs[0]) ? $rs[0] : '';
$room_name = isset($rs['room']) ? $rs['room'] : '';

// Update current room in session table to indicate user is in this room
if ($room_id != '') {
	$sid_escaped = mysqli_real_escape_string($link, session_id());
	$link->query("UPDATE `session` SET current_room = '" . mysqli_real_escape_string($link, $room_id) . "' WHERE sid = '$sid_escaped'");
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $room_id != '') {
	...
	if (trim($Message) !== '') {
		...
		$room_id_esc = intval($room_id);
		
		$insQuery = "INSERT INTO chat_messages (room_id, sender, message, reply_to) VALUES ($room_id_esc, '$Name_esc', '$Message_esc', $replyTo)";
...
<script> setActive("chat"); setActive("chatlobby"); </script>
		...
		<div class="col-md-3 d-none d-md-flex flex-column p-0 bg-light border-right h-100 mobile-pane-overlay mobile-left-pane">
			...
			<div class="flex-grow-1 overflow-y-auto p-2" style="overflow-y: auto;">
				<div class="list-group list-group-flush">
					...
					<a href="chat_rooms.php" class="list-group-item list-group-item-action d-flex align-items-center py-2 px-3 border-0 rounded mb-1 text-dark">
						<i class="fas fa-comments mr-2 text-secondary" style="width: 18px;"></i>
		...
		<div class="col-12 col-md-6 d-flex flex-column p-0 h-100 bg-white">
			...
			<div class="p-3 bg-white border-bottom d-flex justify-content-between align-items-center">
				<div class="d-flex align-items-center">
					...
					<div class="bg-primary rounded-circle mr-2 d-flex align-items-center justify-content-center text-white font-weight-bold shadow-xs" style="width: 32px; height: 32px; font-size: 14px;">
						#<?php echo htmlentities($room_id); ?>
					</div>
					<div>
						<h6 class="font-weight-bold text-dark mb-0"><?php echo htmlentities($room_name); ?></h6>
						<small class="text-success font-weight-bold"><i class="fas fa-circle mr-1 animate-pulse" style="font-size: 8px;"></i>Live Room</small>
					</div>
				</div>
				<div class="d-flex">
			...
			<div class="flex-grow-1 p-3 chat-feed-container" style="overflow-y: auto; background-color: #f7f9fa;">
				<!-- Arrival Log -->
				<div class="chat-system-messages small text-muted mb-3 border-bottom pb-2">
					<div><i class="fas fa-user-plus text-secondary mr-1"></i><b><?php echo htmlentities($currentUser); ?> has joined Chat Room #<?php echo htmlentities($room_id); ?>.</b></div>
				</div>
				...
				<div class="chat-messages-container">
					<?php
						...
						$room_id_esc = intval($room_id);
						$query = $link->query("SELECT * FROM chat_messages WHERE room_id = $room_id_esc ORDER BY id ASC");
						while ($msgRow = mysqli_fetch_array($query)) {
							$msgId = intval($msgRow['id']);
							$sender = $msgRow['sender'];
							$message = $msgRow['message'];
		...
		<div class="col-md-3 d-none d-md-flex flex-column p-0 bg-light border-left h-100 mobile-pane-overlay mobile-right-pane">
			...
			<div class="flex-grow-1 overflow-y-auto p-3 online-panel-content" style="overflow-y: auto;">
				<?php
					$extg = $link->query("SELECT COUNT(*) FROM `session` WHERE guest=1 AND last_active >= DATE_SUB(NOW(), INTERVAL 60 SECOND)");
					if (!$extg) {
						error_log("Database error in room.php line 612: " . $link->error);
					}
					$rstg = $extg ? mysqli_fetch_array($extg) : [0];
					
					$extu = $link->query("SELECT COUNT(DISTINCT username) FROM `users_sessions` WHERE logout_time IS NULL AND last_active >= DATE_SUB(NOW(), INTERVAL 60 SECOND)");
					if (!$extu) {
						error_log("Database error in room.php line 615: " . $link->error);
					}
					$rstu = $extu ? mysqli_fetch_array($extu) : [0];
				...
				<div class="d-flex justify-content-between align-items-center mb-3">
					<span class="font-weight-bold text-success">Total Active:</span>
					<span class="badge badge-success px-3 py-1 font-weight-bold h6 mb-0"><?php echo number_format($totals); ?></span>
				</div>
