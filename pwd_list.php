<?php
	...
	$ex=$link->query("select * from pwd where (
		idn like'%".$value."%' or
	...
	$ex1=$link->query("select * from pwd where (
		idn like'%".$value."%' or
		name_fam like'%".$value."%' or
		name_1st like'%".$value."%' or
		name_mid like'%".$value."%' or
	...
	<div class="container">
		<div class="row">
			<div class="col justify-content-between align-items-center text-center">
				...
				<select class="swid spad bmargin btn btn-sm btn-outline-primary" onchange="if(this.value=='All barangays')jump('pwd_list.php'); else jump('pwd_list.php?barangays='+this.value+'')">
				...
				<button class="bmargin btn btn-sm btn-outline-info" type="button" onclick="getID('t_search').value='';jump('pwd_list.php')"><i class="fa fa-sync tpad"></i> <x class="thid">Refresh</x></button>
				<?php
        ...
        <table align="center" style="margin:0 auto;">
            <tr>
                ...
                <td class="px-3 text-center">
                    <b style="font-size:16px;text-transform:uppercase">List of Persons With Disability (PWD)</b>
    ...
    <div class="card border-0 shadow-sm rounded-lg overflow-hidden">
        <div class="card-header bg-info text-white py-3 d-flex align-items-center justify-content-between">
            <h6 class="mb-0 font-weight-bold">
                <i class="fas fa-wheelchair mr-2"></i>PWD List
            </h6>
                ...
                <?php
                    ...
                    while($rs=mysqli_fetch_array($ex)){
                        if($ctr<=$rec){
                            ...
                            echo "<tr class='tr-hover' style='cursor:pointer;' onclick=\"jump('pwd_pds.php?pwd=".$rs[0]."')\">";
                            echo "<td class='text-center font-weight-bold text-muted'>$i.</td>";
                            echo "<td class='font-weight-bold text-info text-uppercase'>$fullname_disp</td>";
                            echo "<td class='text-center'><span class='badge badge-info px-2 py-1'>".sprintf("%04d", $rs[0])."</span></td>";
                            echo "<td class='font-weight-bold'>".($rs["disability"] ? htmlspecialchars($rs["disability"]) : "N/A")."</td>";
