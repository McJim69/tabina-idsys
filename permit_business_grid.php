...
<script> setActive("businessgrid"); </script>
	...
	<div class="container">
		<div class="row">
			<div class="col justify-content-between align-items-center text-center">
				...
				<button class="bmargin btn btn-sm btn-outline-dark" type="button" onclick="jump('permit_business_list.php')"><i class="fa fa-list tpad"></i> </i> <x class="thid">List View</x></button>
				<select class="swid bmargin btn btn-sm btn-outline-dark" onchange="if(this.value=='All barangays')jump('permit_business_grid.php'); else jump('permit_business_grid.php?barangays='+this.value+'')">
				...
				<button class="bmargin btn btn-sm btn-outline-info" type="button" onclick="getID('t_search').value='';jump('permit_business_grid.php')"><i class="fa fa-sync tpad"></i> <x class="thid">Refresh</x></button>
				<?php
					if(!isset($_SESSION['user'])){
						echo"";
					}else
						echo"
					<a rel='facebox' href='permit_business_add.php'><button class='bmargin btn btn-sm btn-outline-success' type='button'><i class='fa fa-plus tpad'></i> <x class='thid'>Add Permit</x></button></a>";
				?>			
			</div>				
		</div>
	</div>
		...
		<?php
			...
			while($rs=mysqli_fetch_array($ex)){
				...
				if(isset($_POST["b_remove_$rs[0]"])){
					$link->query("delete from permit_business where idn='$rs[0]'");
					jump("");
				}
				
				if(isset($_POST["b_upImg_$rs[0]"])){
					move_uploaded_file($_FILES["b_file_$rs[0]"]["tmp_name"], "images/permit_business/$rs[0].jpg");
					$link->query("update permit_business set ispicset=1 where idn='$rs[0]'");
					jump("");
				}

				$photo_path = 'images/polls.png';
				$img_style = "opacity: 0.3;";
				if(file_exists("images/permit_business/$rs[0].jpg")){
					$photo_path = "images/permit_business/$rs[0].jpg?" . date("h:i:s");
					$img_style = "";
				}
				...
				<div class="col-xl-3 col-lg-4 col-md-6 col-sm-12 mb-4" id="div_<?php echo $rs[0]; ?>">
					<div class="card permit-card h-100 shadow-sm border-2">
						<div class="permit-card-img-container">
							...
							<!-- Image -->
							<img src="<?php echo $photo_path; ?>" alt="Permit Photo" onclick="jump('permit_business_pds.php?permit_business=<?php echo $rs[0]; ?>')" style="cursor:pointer; <?php echo $img_style; ?>" />
							
							<!-- Hover actions overlay -->
							<div class="permit-hover-actions">
								<button type="button" class="btn btn-sm btn-primary btn-block my-1" onclick="jump('permit_business_pds.php?permit_business=<?php echo $rs[0]; ?>')">
								...
								<?php if ($is_admin): ?>
									...
									<a rel="facebox" href="permit_business_edit_form.php?permit_business=<?php echo $rs[0]; ?>" class="btn btn-sm btn-warning btn-block my-1">
										<i class="fas fa-edit mr-1"></i> Edit Details
									</a>
									<button type="button" class="btn btn-sm btn-danger btn-block my-1" onclick="deleteRecord('permit_business', <?php echo $rs[0]; ?>, 'div_<?php echo $rs[0]; ?>')">
						...
						<div class="card-body p-3 d-flex flex-column justify-content-between">
							<div>
								...
								<div class="small mb-1 text-truncate">
									<span class="text-muted">BP No:</span>
								...
								<div class="small text-muted mb-1 text-truncate">
									Owner: <strong class="text-success text-uppercase"><?php echo str_replace($search_val, $rep, $fullname); ?></strong>
								</div>
								...
								<!-- Word Amount Paid -->
								<div class="small text-muted mb-1 font-italic border-top pt-2">
									Amount: <strong><?php echo convert_int_txt($rs["oramount"]); ?></strong> Pesos Only
								</div>
								...
								<div class="mt-2 pt-2 border-top">
									<div class="d-flex align-items-center justify-content-between">
										...
										<select class="form-control form-control-sm border-0 font-weight-bold status-select" data-table="permit_business" data-id="<?php echo $rs['idn']; ?>" style="width: 110px; border-radius: 20px; font-size: 11px; position: relative; z-index: 10 !important;">
