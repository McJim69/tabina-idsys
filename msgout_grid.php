...
<script> setActive("msgoutgrid"); </script>
	...
	<div class="container">
		<div class="row">
			<div class="col justify-content-between align-items-center text-center">
				...
				<button class="bmargin btn btn-sm btn-outline-dark" type="button" onclick="jump('msgout_list.php')"><i class="fa fa-list tpad"></i> </i> <x class="thid">List View</x></button>
				<select class="swid spad bmargin btn btn-sm btn-outline-dark" onchange="if(this.value=='Message To')jump('msgout_grid.php'); else jump('msgout_grid.php?msgout='+this.value+'')">
					...
					<?php
						...
						while($rs2=mysqli_fetch_array($ex2)){
							echo "<option ";
								if($_GET["msgout"]===$rs2[0])
								echo "selected";
							echo">$rs2[0]</option>";
						}
					?>
				</select>
				<button class="bmargin btn btn-sm btn-outline-info" type="button" onclick="getID('t_search').value='';jump('msgout_grid.php')"><i class="fa fa-sync tpad"></i> <x class="thid">Refresh</x></button>
				<?php
					if(!isset($_SESSION['user'])){
						echo"";
					}else
						echo"
					<a rel='facebox' href='msgout_add.php'><button class='bmargin btn btn-sm btn-outline-success' type='button'><i class='fa fa-plus tpad'></i> <x class='thid'>Add Outgoing Message</x></button></a>";
				?>			
			</div>				
		</div>
	</div>
...
<div class="container py-4 grid">
	<div class="row"> 
		<?php
			...
			$msg="";
			if($_GET["msgout"]!="Message To" && $_GET["msgout"]!="") {
				$msg=" and msg_to='".$_GET["msgout"]."'";
			}
			...
			while($rs=mysqli_fetch_array($ex)){
				...
				if(isset($_POST["b_remove_$rs[0]"])){
					$link->query("delete from msgout where idn='$rs[0]'");
					jump("");
				}
				
				if(isset($_POST["b_upImg_$rs[0]"])){
					move_uploaded_file($_FILES["b_file_$rs[0]"]["tmp_name"], "images/msgout/$rs[0].jpg");
					$link->query("update msgout set ispicset=1 where idn='$rs[0]'");
					jump("");
				}

				$photo_path = 'images/back_ca.jpg';
				if(file_exists("images/msgout/$rs[0].jpg")){
					$photo_path = "images/msgout/$rs[0].jpg?" . date("h:i:s");
				}

				?>
				<div class="col-xl-3 col-lg-4 col-md-6 col-sm-12 mb-4" id="div_<?php echo $rs[0]; ?>">
					<div class="card msgout-card h-100 shadow-sm border-2">
						<div class="msgout-card-img-container">
							<!-- Index Badge -->
							...
							<!-- Image -->
							<img src="<?php echo $photo_path; ?>" alt="Message Photo" onclick="jump('msgout_pds.php?msgout=<?php echo $rs[0]; ?>')" style="cursor:pointer;" />
							
							<!-- Hover actions overlay -->
							<div class="msgout-hover-actions">
								<button type="button" class="btn btn-sm btn-primary btn-block my-1" onclick="jump('msgout_pds.php?msgout=<?php echo $rs[0]; ?>')">
								...
								<?php if ($is_admin): ?>
									...
									<a rel="facebox" href="msgout_edit_form.php?msgout=<?php echo $rs[0]; ?>" class="btn btn-sm btn-warning btn-block my-1">
										<i class="fas fa-edit mr-1"></i> Edit Details
									</a>
									<button type="button" class="btn btn-sm btn-danger btn-block my-1" onclick="msgout_delete('<?php echo $rs[0]; ?>')">
...
<script>
	function msgout_delete(idn){	
		if(confirm("Are you sure you want to Remove this msgout?")){
			...
			xmlhttp.open("GET","msgout_delete.php?idn="+idn,true);
			xmlhttp.send();
		}
	}
</script>